...
public class List {
    ...
    private boolean init() {
        if (!inited) {
            ...
            }else if (orderBy != null && orderBy.equals("pwcReleaseDateAsc")) { //reverse the order of the list to ascending 
                Collections.sort(pages, new PageComparator<Page>("pwcReleaseDate"));
...
//            log.error("Subha: Testing List Comp"+property+" - "+comp+" - "+isDateProperty);
            return (comp != 0 && isDateProperty) ? -comp : comp;
        }

        private String getKey(P p) {
//        	log.error("Subha: Testing List Key"+property+" - "+p.getProperties().get(property, "")+" - "+isDateProperty+" - "+props.get(property,""));
